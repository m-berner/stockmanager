...
onBeforeMount(async () => {
  ...
  const onStorageChange = (
    ...
    // const { fetchExchangesData } = useFetchApi()
    // const urls = { url: `${browser.runtime.getURL(CONS.RESOURCES.INDEX)}*` }
    // const foundTabs = await browser.tabs.query(urls)
    // const appTab = foundTabs.length > 0 ? foundTabs[0].id ?? -1 : -1
    switch (true) {
